...
class REINFORCE:
    ...
    def __init__(self, obs_space_dims: int, action_space_dims: int, lr, gamma, eps):
        ...
        self.writer = SummaryWriter(writeSummary("_lr_"+str(self.learning_rate),"_gamma_"+str(self.gamma), "_eps_"+str(self.eps)))
...
for seed in [1]:  # Fibonacci seeds
    ...
    for lr in [0.001]:#[1e-6, 5e-6, 1e-5, 5e-5, 1e-4]:
        for gamma in [0.99]:#[0.99, 0.95, 0.8]:
            for eps in [1]:#[1, 0.5, 1e-2, 1e-3, 1e-8]:
